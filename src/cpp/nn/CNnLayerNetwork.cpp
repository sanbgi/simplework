...
int CNnLayerNetwork::initNetwork() {
    ...
    for(int i=0, nLayers=m_arrLayers.size(); i<nLayers; i++) {
        ...
        layerCtx.iInputVarIndex = solveCtx.iInVar;
        layerCtx.iOutputVarIndex = solveCtx.iOutVar;
        layerCtx.nInputTensorSize = solveCtx.spInVar->getSize();
        layerCtx.nOutputTensorSize = spDimension.dataSize();
...
int CNnLayerNetwork::evalT(const STensor& spBatchIn, STensor& spBatchOut) {
    ...
    while(pInData < pInDataEnd) {
        ...
        for(int iLayer=0; iLayer<nLayers; iLayer++) {
            ...
            //
            PSolveVar* pInVar = pVars + pLayer->iInputVarIndex;
            if(iLayer == 0) {
                pInVar->data = pInData;
            }
            else {
...
int CNnLayerNetwork::learnT(const STensor& spBatchOut, const STensor& spBatchOutDeviation, STensor& spBatchIn, STensor& spBatchInDeviation) {
    ...
    while(pInData < pInDataEnd) {
        ...
        for(int iLayer=nLayers-1; iLayer>=0; iLayer--){
            ...
            // 准备输入数据指针，比较复杂，大体逻辑是：
            //  1，如果是第一层，则指向原始输入指针
            //  2，如果不是第一层，则指向上一层输出的指针，但分两种情况：
            //      2.1，如果上一层是正常模式或者序列模式，则没问题
            //      2.2，如果上一层是Batch模式，则需要从指向最后一个的指针计算出整个BATCH的开始指针
            //
            PSolveVar * pInVar = pVars + pLayer->iInputVarIndex;
            if(iLayer == 0) {
                pInVar->data = pInData;
