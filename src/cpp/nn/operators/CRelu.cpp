...
//
class CRelu : public CObject, IKernalOperator {
    SIMPLEWORK_INTERFACE_ENTRY_ENTER(CObject)
        SIMPLEWORK_INTERFACE_ENTRY(IKernalOperator)
    SIMPLEWORK_INTERFACE_ENTRY_LEAVE(CObject)

public://Factory
    static const char* __getClassKey() { return "sw.nn.Relu"; }
...
#include "Relu.cl"
    ...
    static void floatEval(const PKernalCtx* pCtx, int nArgs, PKernalVariable pArgs[]) {
        ...
        sKernel.floatEval(
                _KArg(CKernelWraper::PReluParameter*,0), 
                _KArg(int,1),
...
SIMPLEWORK_SINGLETON_FACTORY_AUTO_REGISTER(CRelu, CRelu::__getClassKey())
